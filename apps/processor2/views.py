...
#                         return redirect(f"{reverse('quickbooks_login')}?next=add_processor2")
...
#                         return render(request, 'processor2/add_processor2.html', {'form': form})
...
def add_processor2(request):
    ...
    try:
        ...
        if request.user.is_superuser or 'SubAdmin' in request.user.get_role() or 'SuperUser' in request.user.get_role():
            ...
            form = ProcessorForm2(processor_type_name="T2")
            context['form'] = form
            if request.method == 'POST':
                    fein = request.POST.get('fein')
                    entity_name = request.POST.get('entity_name')
...
#             from apps.quickbooks_integration.models import QuickBooksToken
#             from apps.quickbooks_integration.views import refresh_quickbooks_token, create_vendor, update_vendor, get_vendor_data
#             success_url = reverse('processor2_update', kwargs={'pk': pk})
        ...
        if request.user.is_superuser or 'SubAdmin' in request.user.get_role() or 'SuperUser' in request.user.get_role():       
            ...
            if request.method == "POST":
                ...
                if bin_pull and not data.get("save"): 
                    if sku:                   
                        context["milled_value"] =  calculate_milled_volume(selected_crop, int(bin_pull), "T2", sku) 
                    ...
                    save_shipment_management = ShipmentManagement(shipment_id=shipment_id,processor_idd=bin_pull,processor_e_name=processor_e_name, sender_processor_type="T2", bin_location=bin_pull, crop=selected_crop, variety=context.get("variety"),
