...
#                         return redirect(f"{reverse('quickbooks_login')}?next=add_processor4")
...
#                         return render(request, 'processor4/add_processor4.html', {'form': form})
...
def add_processor4(request):
    ...
    try:
        ...
        if request.user.is_superuser or 'SubAdmin' in request.user.get_role() or 'SuperUser' in request.user.get_role():
            ...
            form = ProcessorForm2(processor_type_name="T4")
            context['form'] = form
            if request.method == 'POST':
                print(request.POST)
                fein = request.POST.get('fein')
...
#                 from apps.quickbooks_integration.models import QuickBooksToken
#                 from apps.quickbooks_integration.views import refresh_quickbooks_token, create_vendor, update_vendor, get_vendor_data
#                 success_url = reverse('update_processor4', kwargs={'pk': pk})
        ...
        if request.user.is_superuser or 'SubAdmin' in request.user.get_role() or 'SuperUser' in request.user.get_role():
            ...
            if request.method == "POST":
                ...
                if bin_pull_type == "T1":
                    ...
                    context["sender_sku_id_list"] = get_sku_list(int(bin_pull), bin_pull_type)["data"]
                ...
                if bin_pull and not data.get("save"):                
                    ...
                    if processor2_id:                        
                        ...
                        context["receiver_sku_id_list"] = get_sku_list(int(processor2_id.split()[0]),"T4")["data"] 
...
@login_required()  # show processor4 list
