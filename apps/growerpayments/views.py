...
def grower_payments_table_csv_download(request):
    ...
    if request.user.is_superuser or 'SubAdmin' in request.user.get_role() or 'SuperUser' in request.user.get_role():
        ...
        entry = EntryFeeds.objects.all().values("grower_id")
        total_grower_id = [i["grower_id"] for i in entry]
        # Fixed Market Price ..................
        bale = BaleReportFarmField.objects.filter(ob2__in = total_grower_id).values("dt_class","bale_id","ob3","farm_name","field_name","net_wt",
                                                                                    "classing__processor__entity_name","level","ob2")
        for i in bale :
            delivery_date = i["dt_class"]
            delivery_id = i["bale_id"]
            grower_name = i["ob3"]
            ...
            delivery_lbs = i["net_wt"]
            processor = i["classing__processor__entity_name"]
            classs = i["level"]
        ...
        #     # 27-03-23
            check_entry = EntryFeeds.objects.filter(grower_id = i["ob2"]).only("id")
            if len(check_entry) == 0 :
                continue
            elif len(check_entry) == 1 :
                var = EntryFeeds.objects.get(grower_id = i["ob2"])
            elif len(check_entry) > 1 :
                check_entry_with_date = EntryFeeds.objects.filter(grower_id = i["ob2"],from_date__lte=finale_date,to_date__gte=finale_date).only("id")
                check_entry_with_no_date = EntryFeeds.objects.filter(grower_id = i["ob2"],from_date__isnull=True,to_date__isnull=True).only("id")
                if check_entry_with_date.exists() :
                    check_entry_id = [i["id"] for i in check_entry_with_date][0]
                    var = EntryFeeds.objects.get(id = check_entry_id)
                elif check_entry_with_no_date.exists() :
            ...
            if classs != "None":
                total_price = float(cpb_lbs) + float(sp_lbs) + float(qp_lbs)
                delivered_value = float(delivery_lbs) * float(total_price)
                if delivery_date and finale_date != '' :
                    # date_str = str(delivery_date).split("/")
                    # dd = int(date_str[1])
                    # mm = int(date_str[0])
                    # yy = int(date_str[2])
                ...
                delivered_value = 0.00
                payment_due_date ="N/A"
            writer.writerow([delivery_date,delivery_id,grower_name, crop, '-', farm_name,field, delivery_lbs,processor,
            classs, cpb_lbs, sp_lbs, qp_lbs, total_price, delivered_value, payment_due_date])
            # data = {
            #         'delivery_date':delivery_date,
            #         'delivery_id':delivery_id,
            #         'grower_name':grower_name,
        ...
        for i in grower_shipment :
            ...
            total_price = "{0:.5f}".format(total_price)
            delivered_value = "{0:.4f}".format(delivered_value)
            writer.writerow([delivery_date,delivery_id,grower_name, crop, variety,'-', field, delivery_lbs,processor,
            "-", cpb_lbs, sp_lbs,"-", total_price, delivered_value,payment_due_date])
            # data = {
            #     'delivery_date':delivery_date,
            #     'delivery_id':delivery_id,
            #     'grower_name':grower_name,
...
def grower_payments_list(request):
    ...
    if request.user.is_superuser or 'SubAdmin' in request.user.get_role() or 'SuperUser' in request.user.get_role():
        ...
        context['selectedGrower'] = selectedGrower
        context['selectedCrop'] = selectedCrop
        if context['selectedGrower'] :
            fld = Field.objects.filter(grower_id=selectedGrower.id)
            context['selectedCrop'] = fld.first().crop
