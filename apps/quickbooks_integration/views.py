...
def vendor_list(request):
    ...
    try:
        ...
        if not raw_vendor_list:
            print("API call returned no data, using empty list.")
            raw_vendor_list = []
    except Exception as e:
        print(f"API call failed: {e}")
        raw_vendor_list = []

    print(raw_vendor_list)
...
def customer_list(request):
    ...
    for full_customer_data in raw_customer_list:
        ...
        if not Customer.objects.filter(Q(quickbooks_id=customer_id) | Q(name=customer_name)).exists():
...
def item_list(request):
    ...
    for full_item_data in raw_item_list:
        ...
        if item_type == 'Inventory': 
            ...
            item_id = item["id"]
            name = item["class_name"].upper()
            item_name = item["item_name"]
            item_ = f'{name}-{item_name}'
            if not ShipmentItem.objects.filter(Q(quickbooks_id=item_id) | Q(item=item_)).exists():
