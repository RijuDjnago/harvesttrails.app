...
from apps.docusign.api_type import EXAMPLES_API_TYPE
from apps.docusign.error_handlers import process_error
from apps.docusign.jwt_helpers import get_jwt_token, get_private_key
...
class DSClient:
    ...
    def _auth_code_grant(cls):
        ...
        cls.ds_app = oauth.create_client('docusign')
    ...
    def _jwt_auth(cls):
        ...
        try:
            cls.ds_app = get_jwt_token(private_key, use_scopes, DS_JWT["authorization_server"], DS_JWT["ds_client_id"],
                                       DS_JWT["ds_impersonated_user_id"])
            return redirect(reverse("ds_callback"))
            ...
            if "consent_required" in body:
                ...
                redirect_uri = DS_CONFIG["app_url"] + reverse("ds_callback")
    ...
    @classmethod
    def login(cls, auth_type, request):
        if auth_type == "code_grant":
            return cls.get(auth_type).authorize_access_token(request)
        elif auth_type == "jwt":
    ...
    def get_api_client(cls):
        api_client = ApiClient()
        api_client.host = f"{DS_JWT['ds_base_url']}/restapi"
