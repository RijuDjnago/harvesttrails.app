...
def index(request):
    if request.method == 'POST':
        ...
        if action == 'go':
            user_ids = request.POST.getlist('checkbox')
            action_value = request.POST.get('act')
            if action_value == "active":
                for k in user_ids:
                    User.objects.filter(id=int(k)).update(is_active = True)
            if action_value == "inactive":
                for i in user_ids:
                    User.objects.filter(id=int(i)).update(is_active = False)
