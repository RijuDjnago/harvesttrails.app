...
def test(request):
    ...
    field = Field(name=name,farm_id=farm,grower_id=grower,acreage=acreage,crop=crop,batch_id=batch_id,fsa_farm_number=fsa_farm_number,fsa_tract_number=fsa_tract_number,
    ...
    field_save_new = FieldUpdated(field_id=field.id,name=field.name,crop_year="2024",fsa_farm_number=fsa_farm_number,
                     fsa_tract_number=fsa_tract_number,fsa_field_number=fsa_field_number,crop=crop,
                     yield_per_acre=yield_per_acre,total_yield=total_yield,crop_tech=crop_tech,
                     latitude=latitude,longitude=longitude,batch_id=batch_id,acreage=acreage,variety=variety,grower_id=grower,farm_id=farm)
    field_save_new.save()
...
class FieldCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        scrop_year = form.cleaned_data.get('scrop_year')
        print(scrop_year,scrop_year)
...
def fieldActivity(request):
    ...
    print("hidden_field_id.................",hidden_field_id)
    print("hidden_field_id",hidden_field_id)
    # hidden_field_id = 2508
    # Field Update Data.. 
    hidden_field_name = request.POST.get('hidden_field_name')
    print("hidden_field_name",hidden_field_name)
    ...
    if hidden_field_updated_field_id and hidden_field_crop_year :
        ...
        if len(hidden_field_latitude) != 0 :
            field_save_old.latitude = hidden_field_latitude
            field_save_old.save()
            fieldEditLogTable(userid, hidden_field_id, f'latitude = {hidden_field_latitude}')
        if len(hidden_field_longitude) != 0 :
            field_save_old.longitude = hidden_field_longitude
            field_save_old.save()
            fieldEditLogTable(userid, hidden_field_id, f'longitude = {hidden_field_longitude}')

        if len(hidden_field_crop) != 0 :
            field_save_old.crop = hidden_field_crop
            field_save_old.save()
        ...
        hidden_field_stand_count = hidden_field_stand_count if hidden_field_stand_count else field_obj.stand_count
        hidden_field_plant_date = hidden_field_plant_date if hidden_field_plant_date else field_obj.plant_date
        hidden_field_harvest_date = hidden_field_harvest_date if hidden_field_harvest_date else field_obj.harvest_date
        hidden_field_crop = field_obj.crop
        print("................................................")
        # field_save_new = FieldUpdated(field_id=hidden_field_id,name=hidden_field_name,crop_year="2024",fsa_farm_number=hidden_field_fsa_farm_number,
        ...
        field_save_new = FieldUpdated(field_id=hidden_field_id,name=field_obj.name,crop_year="2024",fsa_farm_number=field_obj.fsa_farm_number,
                     fsa_tract_number=field_obj.fsa_tract_number,fsa_field_number=field_obj.fsa_field_number,crop=field_obj.crop,
                     yield_per_acre=field_obj.yield_per_acre,total_yield=field_obj.total_yield,crop_tech=field_obj.crop_tech,acreage=field_obj.acreage,
                     previous_crop=field_obj.previous_crop,stand_count=field_obj.stand_count,plant_date=field_obj.plant_date,
                     harvest_date=field_obj.harvest_date,variety=field_obj.variety,batch_id=field_obj.batch_id,eschlon_id=field_obj.eschlon_id,
                     water_saving=field_obj.water_saving,gal_water_saved=field_obj.gal_water_saved,water_lbs_saved=field_obj.water_lbs_saved,co2_eq_reduced=field_obj.co2_eq_reduced,
                     increase_nitrogen=field_obj.increase_nitrogen,ghg_reduction=field_obj.ghg_reduction,land_use_efficiency=field_obj.land_use_efficiency,grower_premium_percentage=field_obj.grower_premium_percentage,
                     grower_dollar_premium=field_obj.grower_dollar_premium,latitude=field_obj.latitude,longitude=field_obj.longitude)
        field_save_new.save()
        fieldEditLogTable(userid, hidden_field_id, f'crop_year = 2024, Data Added')
...
def update_field_2024(request,pk):
    ...
    if request.user.is_superuser or 'SubAdmin' in request.user.get_role() or 'SuperUser' in request.user.get_role():
        ...
        check_field = Field.objects.filter(id=pk)
        if check_field.exists():
            grower_id = check_field.first().grower.id
            context["update_field"] = Field.objects.get(id=pk)
            context["farms"] = Farm.objects.filter(grower_id=grower_id)
            context["growers_dpdwn"] = Grower.objects.filter(id=grower_id)
            context["selected_grower"] = context["growers_dpdwn"].first()
            context["selected_scrop_year"] = "2024"
        ...
        check_field = Field.objects.filter(grower_id=grower_id,id=pk)
        if check_field.exists():
            context["update_field"] = Field.objects.get(id=pk)
            context["farms"] = Farm.objects.filter(grower_id=grower_id)
            context["growers_dpdwn"] = Grower.objects.filter(id=grower_id)
    ...
    print("pk",pk)
    return render(request, "field/update_field_2024.html",context)
...
class FieldUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs,):
        ...
        if 'Grower' in self.request.user.get_role() and not self.request.user.is_superuser:
            ...
            update_field = FieldUpdated.objects.filter(field_id=pk,crop_year=scrop_year)
            if update_field.exists():
                context["update_field"] = update_field.first()
            else:
                print("scrop_year",scrop_year,"no data Found")
                context["update_field"] = Field.objects.get(id = pk)
                context["new_update_field"] = FieldUpdated.objects.filter(field_id=pk).first()
                
                messages.error(self.request, f"Field Data not found for crop year {scrop_year}. The system only have crop year {context['new_update_field'].crop_year} data.")
                scrop_year = context['new_update_field'].crop_year

            if scrop_year and scrop_year != "" :
                context["selected_scrop_year"] = scrop_year
                checkUpdate_field = FieldUpdated.objects.filter(field_id=pk,crop_year=scrop_year)
                if checkUpdate_field.exists() :
                    new_update_field = checkUpdate_field.first()
                ...
                elif scrop_year != "" and scrop_year !="2022" and scrop_year !="2023" :
                    scrop_year = "2024"
                    get_field = Field.objects.get(id = pk)
                    saveUpdate_field = FieldUpdated(field_id=pk,crop=get_field.crop,name=get_field.name,crop_year=scrop_year,farm_id=get_field.farm.id,
                                                    grower_id=get_field.grower.id,batch_id=get_field.batch_id,acreage=get_field.acreage,
                                                    fsa_farm_number=get_field.fsa_farm_number,fsa_tract_number=get_field.fsa_tract_number,
                                                    fsa_field_number=get_field.fsa_field_number,yield_per_acre=get_field.yield_per_acre,total_yield=get_field.total_yield,
                                                    crop_tech=get_field.crop_tech,previous_crop=get_field.previous_crop,stand_count=get_field.stand_count,
                                                    plant_date=get_field.plant_date,harvest_date=get_field.harvest_date,eschlon_id=get_field.eschlon_id,
                                                    water_saving=get_field.water_saving,gal_water_saved=get_field.gal_water_saved,water_lbs_saved=get_field.water_lbs_saved,
                                                    co2_eq_reduced=get_field.co2_eq_reduced,increase_nitrogen=get_field.increase_nitrogen,ghg_reduction=get_field.ghg_reduction,
                                                    land_use_efficiency=get_field.land_use_efficiency,grower_premium_percentage=get_field.grower_premium_percentage,
                                                    grower_dollar_premium=get_field.grower_dollar_premium)
                    saveUpdate_field.save()
                    id_checkUpdate_field = [saveUpdate_field.id]
                    context["update_field"] = saveUpdate_field
...
# @login_required()
# def fieldUpdateView(request,pk):
#     context = {}
#     if 'Grower' in request.user.get_role() and not request.user.is_superuser:
#         update_field = Field.objects.get(id = pk)
#         context["update_field"] = update_field
#         farms_data = Farm.objects.filter(id=update_field.farm.id).order_by('name')
#         context["farms"] = farms_data
#         context["growers_dpdwn"] = Grower.objects.filter(
#                 id=update_field.grower.id).order_by('name')
#         if request.method == 'POST':
#             scrop_year = request.POST.get('scrop_year')
#             fld_name = request.POST.get('fname')
...
#             btnvariable = request.POST.get('btnvariable')
...
#         context["growers_dpdwn"] = Grower.objects.filter(
#                 id=update_field.grower.id).order_by('name')
#     return render(request,'field/field_update.html',context)
...
def field_data_update(request):
    if request.user.is_superuser or 'SubAdmin' in request.user.get_role() or 'SuperUser' in request.user.get_role():
        ...
        for i in fields :
            ...
            check_update_field = FieldUpdated.objects.filter(field_id=old_field.id)
            if check_update_field.exists():
                update_field = check_update_field.first() 

                update_field.name = old_field.name
                update_field.crop_year = "2022"
                update_field.farm_id = old_field.farm.id
                update_field.grower_id = old_field.grower.id
                update_field.batch_id = old_field.batch_id
                update_field.acreage = old_field.acreage
                update_field.latitude = old_field.latitude
                update_field.longitude = old_field.longitude
                update_field.fsa_farm_number = old_field.fsa_farm_number
                update_field.fsa_tract_number = old_field.fsa_tract_number
                update_field.fsa_field_number = old_field.fsa_field_number
                update_field.crop = old_field.crop
                update_field.variety = old_field.variety
                update_field.yield_per_acre = old_field.yield_per_acre
                update_field.total_yield = old_field.total_yield
                update_field.crop_tech = old_field.crop_tech
                update_field.previous_crop = old_field.previous_crop
                update_field.stand_count = old_field.stand_count
                update_field.plant_date = old_field.plant_date
                update_field.harvest_date = old_field.harvest_date
                update_field.eschlon_id = old_field.eschlon_id
                update_field.water_saving = old_field.water_saving
                update_field.gal_water_saved = old_field.gal_water_saved
                update_field.water_lbs_saved = old_field.water_lbs_saved
                update_field.co2_eq_reduced = old_field.co2_eq_reduced
                update_field.increase_nitrogen = old_field.increase_nitrogen
                update_field.ghg_reduction = old_field.ghg_reduction
                update_field.land_use_efficiency = old_field.land_use_efficiency
                update_field.grower_premium_percentage = old_field.grower_premium_percentage
                update_field.grower_dollar_premium = old_field.grower_dollar_premium
                update_field.save()

            else:
                print("ELSE")
                update_field = FieldUpdated(field_id=old_field.id,name=old_field.name,crop_year="2022",farm_id = old_field.farm.id,
                                ...
                                harvest_date=old_field.harvest_date,grower_id = old_field.grower.id,batch_id = old_field.batch_id,
                                acreage = old_field.acreage,latitude = old_field.latitude,longitude = old_field.longitude,eschlon_id = old_field.eschlon_id,
                                water_saving = old_field.water_saving,gal_water_saved = old_field.gal_water_saved,water_lbs_saved = old_field.water_lbs_saved,
                                co2_eq_reduced = old_field.co2_eq_reduced,increase_nitrogen = old_field.increase_nitrogen,ghg_reduction = old_field.ghg_reduction,
                                land_use_efficiency = old_field.land_use_efficiency,grower_premium_percentage = old_field.grower_premium_percentage,
                                grower_dollar_premium = old_field.grower_dollar_premium)
                update_field.save()
                print("else")
